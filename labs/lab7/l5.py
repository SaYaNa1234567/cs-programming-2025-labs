...
reports_we_need_to_destroy = list(filter(lambda r: 'http' in r["text"], reports))

destroyed = list(map(lambda r: {
    "author": r["author"],
    "text": r["text"].replace('http://', '[ДАННЫЕ УДАЛЕНЫ] ').replace('https://', '[ДАННЫЕ УДАЛЕНЫ] ')
}, reports_we_need_to_destroy))
